...
namespace LG {
    ...
    VKDevice::VKDevice(EngineWindow& m_window)
        : m_window{ m_window }
    {
        CreateInstance();
        SetupDebugMessenger();
    ...
    uint32_t VKDevice::FindMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags m_properties)
    {
        ...
        {
            if ((typeFilter & (1 << i)) &&
                (memProperties.memoryTypes[i].propertyFlags & m_properties) == m_properties) 
            {
                return i;
            }
        }
