...
namespace LG
{
    ...
    void VKPipeline::CreateRenderPass(const PipelineConfigInfo& info)
    {
        ...
        attachment.format = info.swapChainImageFormat;
        ...
        imguisubpass.pColorAttachments = &color_attachment;
        ...
        imguiPassInfo.pAttachments = &attachment;
        imguiPassInfo.subpassCount = 1;
        imguiPassInfo.pSubpasses = &imguisubpass;
        imguiPassInfo.dependencyCount = 1;
        imguiPassInfo.pDependencies = &dependency;

        if (vkCreateRenderPass(m_device, &imguiPassInfo, nullptr, &m_imguiRenderPass) != VK_SUCCESS)
        {
            throw std::runtime_error("failed to create render pass!");
        }
    }
